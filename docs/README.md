...
- Community tabs are now ordered `Overview`, `Board members`, `Events`, and (for `ADMIN`/`SUPER_ADMIN`) `Diplomas`; tab labels include icons. `Overview` shows icon-backed stat cards (users, children, need attention), four donut KPIs (users by role, users by status, children by program including Ilmihal, and children by Ilmihal nivo), plus nivo-level attendance/attention detail with a **program scope filter**, **recorded coverage** (`recorded/children`) column, **status chips** (`Good`, `Watch`, `No data`), and a **hide empty nivos** toggle. The `Children by program` donut center total is shown as **Enrollments** (a child can be counted in multiple programs), while the child KPI card remains distinct-child count.
...
- Edit user dialog now shows existing linked children as a read-only list (with nivo label) above the add-new-child section; the `+ Add child` button is moved below the children list for easier access without scrolling; the remove button for new children is icon-only (compact trash icon).
- Edit user dialog now shows community as a read-only field for ADMIN users who cannot change it (SUPER_ADMIN still gets the dropdown selector).
- Lessons page: Nivo book PDF links now use blue link-styled text with underline-on-hover and an external-link icon instead of the previous green status badge, making them clearly recognizable as clickable previews.
- Edit child dialog now displays the community name as read-only text when the logged-in user is not SUPER_ADMIN, so ADMIN users can see which community a child belongs to during editing.
- Nivo book preview link is now a reusable `NivoBookLink` component (`web/src/features/lessons/nivo-book-link.tsx`) that renders a blue clickable link with FileText + ExternalLink icons; used in both the lessons nivo collapsible and the child details drawer (replacing the old outline "Preview" button).
- Edit user children section: existing linked children are now loaded into the form as editable rows (PATCH on save), not read-only. New children show heading "New child N" while existing children show their name. Adding a new child is disabled until the previous new child has at least first and last name filled. Removing any child (existing or new) shows a frosted overlay confirmation with Remove/Cancel buttons.
- User details drawer (Children tab) now has an "Add child" button that opens the full child form dialog pre-filled with the viewed user as parent, allowing child creation directly from the user drawer without navigating to the children page.
- Child birth date validation enforces a minimum age of 6 years (both Zod schema and HTML date picker `max` attribute) across user form and children form.
