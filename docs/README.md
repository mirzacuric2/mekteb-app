...
- Child drawer **Imam grading**: title with a small **clipboard** icon, then **Completed** switch + hint, then **Mark** label (short on small viewports, full `Mark (1–10)` from `sm`), **numeric input**, and **Save** on **one horizontal row** (horizontal scroll only if the viewport is extremely narrow); **Save** shows a **disk** icon (`Save`) and pending state a **spinner** (`Loader2`). Switch helper text depends on state: **on** → how to mark not completed; **off** → how to mark completed (so it is not backwards when already off).
