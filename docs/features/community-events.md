...
3. Event appears in weekly or monthly calendar slots (based on screen size/placement). In **weekly mode**, **Community → Events** keeps **seven days in one horizontal row** below **`lg`** (`flex-nowrap` + `overflow-x-auto` when needed); from **`lg`**, day wrappers use **`lg:min-w-[7.5rem]`** (empty) and **`lg:min-w-[11rem]`** (dashboard) / **`lg:min-w-[10rem]`** (community) when the day has occurrences; the week row uses **`lg:overflow-x-auto`** so all seven days stay reachable when the row is wider than the card. Read-only calendars do not single-line-clamp event titles (full wrap). The **home dashboard** preview (`dashboardPreview`) differs: **below `lg`** it uses a **vertical stack** (one full-width day card per row); from **`lg`** it uses the same **single row** rules with the same **minimum column width**. Optional `stackWeeklyDays` on `CommunityEventsPanel` forces a vertical stack everywhere (not used in current defaults).
