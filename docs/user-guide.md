...
- Change language from the login card or, after sign-in, from the sidebar language control (changes are saved to your account). You can also open **Settings** (`/app/settings`) from your **user menu** (avatar at the bottom of the sidebar) for the same control and future preferences.
...
- Open **Settings** from the **user menu** (avatar at the bottom of the sidebar) for account preferences (more options will be added over time). The menu is anchored above the avatar so it stays fully visible when the sidebar is narrow/collapsed; click anywhere outside the menu or press **Escape** to close it.
